...
const bplayCaba = async (page, playerData) => {
  const meses = [
  ...
  const pageUrl = process.env.BPLAY_CABA_URL
  ...
  try {
    ...
    await page.waitForSelector('input[name="nationalId"]', { timeout: 30000 });



    //Completar formulario (Paso 1)
    await page.type('input[name="nationalId"]', dni, { delay: 100 }); //Nombre

    await page.click(`label[for="gender_${generoId}"]`); //Genero
    try {
    ...
    await page.locator('#step0to1').click(); //CLICK AL PASO SIGUIENTE  
    ...
    const provinciaNombre = 'Ciudad de Buenos Aires' //toTitleCase(provincia).trim();
    ...
    await page.locator('#step1to2').click();
    /// PASO FINAL
